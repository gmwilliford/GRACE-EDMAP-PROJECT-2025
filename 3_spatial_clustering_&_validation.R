...
# placeholder SY until optimized per-cluster
SY_global <- 0.03
...
gws_lookup <- gws_anomalies_op
names(gws_lookup) <- names(gws_anomalies_op)
...
#Build smoothed monthly series & filter out bunk wells
...
run_clustering <- function(wells_monthly,
  ...
  # 4) Composite distance & clustering
  D_comb <- α * D_t + (1 - α) * (D_s / sd(D_s))
  hc     <- hclust(D_comb, method="ward.D2")
  
  # 5) Score each k by silhouette & fit
...
plot_time_series <- function(cluster_id, wells_clust) {
  ...
  # grab stats from the table we just made
...
# plot just one cluster (or group of clusters)):
