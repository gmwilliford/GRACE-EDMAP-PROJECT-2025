...
#This function will take the GRACE data, GLDAS data, and the model to predict the residuals
#and then apply the correction to the high-resolution data.
...
#Define Extents & Spatial Objects
#Mississippi Embayment
ms_embayment <- extent(-94.5, -87.25, 29.75, 37.25)
ms_embayment_sf <- st_as_sfc(st_bbox(c(xmin = -94.5, xmax = -87.25, ymin = 29.75, ymax = 37.25), crs = st_crs(4326)))
#Ozark Plateaus
ozark_plateaus <- extent(-95.3, -88.6, 34.8, 39.4)
ozark_plateaus_sf<- st_as_sfc(st_bbox(c(xmin = -95.3, xmax = -88.6,ymin = 34.8, ymax = 39.4), crs = st_crs(4326)))
...
ms_corrected <- lapply(
  ...
  function(i) clip_predict_correct(
    ...
    clip_extent     = ms_embayment,
    clip_shape      = ms_embayment_sf
  )
)

ozark_plateaus_corrected <- lapply(
  seq_along(gldas025_anoms_stacked),
  function(i) clip_predict_correct(
    i,
    anom_stack_025 = gldas025_anoms_stacked,
    residuals_hr    = residuals_hr,
    year_months     = year_months,
    clip_extent     = ozark_plateaus,
    clip_shape      = ozark_plateaus_sf
  )
)

corrected_rasters <- Filter(Negate(is.null), ms_corrected)
corrected_rasters_op <- Filter(Negate(is.null), ozark_plateaus_corrected)
names(corrected_rasters_op) <- year_months[!vapply(ozark_plateaus_corrected, is.null, logical(1))]

saveRDS(corrected_rasters_op, "corrected_rasters_op.rds")
corrected_rasters <- readRDS("corrected_rasters_op.rds")

plot(corrected_rasters_op[[1]], main = names(corrected_rasters_op)[1])
...
gws_anomalies_op <- calculate_gws_anomalies(corrected_rasters_op, storage_anoms)
names(gws_anomalies_op) <- names(corrected_rasters_op)
...
# selected region:
region_sf <- ozark_plateaus_sf

# (re)crop state boundaries to that region:
state_boundaries <- st_crop(state_shape, region_sf)
...
generate_stats <- function(month_index) {
  ...
  tws     <- corrected_rasters_op[[month_index]]
  gws     <- gws_anomalies_op[[month_index]]
  storage <- storage_anoms[[month_index]]
  date    <- names(corrected_rasters_op)[month_index]
...
all_stats <- do.call(rbind, lapply(seq_along(corrected_rasters_op), generate_stats))
head(all_stats)
